...
### 5.2 TOML Schema (version 1)

**Top-level keys (required):**

  Field            Type    Description
  --------------------------------------------------------
  schema_version   int     Currently 1.  The reader
...
3. Every required field must be present -- both the
   ...
   A missing field is a hard error with the file
   path, the entry label (when the missing field is
   per-entry), and the field name in the message.
...
6. At least one entry with `label = "isolated"` is
   ...
   compare against it on the same code path.

### 5.3 Sketch (gold, two entries)
...
Public functions:
  ...
  baseline(db)      Convenience for lookup(db,
                    "isolated"); guaranteed to succeed
                    on a valid database (rule 6).
...
### 5.6 Lookup Algorithm (Phase 1)

`makeinput.py` accepts a new CLI argument:

```
-pot LABEL    Apply LABEL uniformly to every atom in
              the structure when emitting the initial
              potential block.  Default: "isolated"
              (preserves current behavior).  Recommended:
              "default_solid" once the augmented
              database is populated.
```

For each unique element symbol in the parsed structure:

1. Construct the path
   `share/atomicPDB/<elem>/s_gaussian_pot.toml`.
2. If the file exists:
   a. Call `initial_potential_db.load(path)`.  On any
      validation error, abort with a message naming
      the file and the failing rule.
   b. Call `lookup(db, LABEL)`:
      - On success, use the returned entry.
      - On KeyError, fall back to `baseline(db)`.
        Emit a warning naming the element and the
        missing label.  `baseline` is guaranteed to
        succeed because rule 6 is enforced at load
        time: a database that lacks `"isolated"`
        fails validation in step 2.a and never
        reaches this branch.
3. If the file does not exist, fall back to the legacy
   `pot1` / `coeff1` reader path.  Emit an info-level
   message that the augmented database is not yet
   populated for this element.
4. Format and write into the Imago input file, in
   today's on-the-wire format: the file's top-level
   `nuclear_z`, `nuclear_alpha`, `covalent_radius`,
   plus the chosen entry's `num_gaussians`, `alpha_min`,
   `alpha_max`, `coefficients`, and `alphas`.

Phase 2 will replace step 2.b with per-site label
selection.  The data source (per-element TOML file) and
the Imago input format are unchanged.
...
The manifest is TOML.  All fields below are required in v1.
Optional fields are deferred to v2 with backward compatibility.
...
3. For each `[[reference_solid]]` in the manifest:
   ...
   c. For each `[[reference_solid.entry]]`:
      ...
      ii.  Construct a `PotentialEntry` (5.2) with the
           manifest-supplied `label` and `description`, the
           run-supplied numerical fields, and the run-supplied
           provenance.
      iii. Insert the entry into the in-memory `ElementDatabase`
...
### 5.9 Open Design Questions (Phase 2)

Deferred to a later DESIGN pass:

- **Per-site label selection.** Algorithm for picking
  a label per atom from local environment, oxidation
  state, or coordination.  Phase 1 uses a single
  global label per run.
- **Interpolation.** Numerical procedure for
  assembling a starting potential when no exact label
  match exists -- e.g., distance-weighted blend across
  nearby labels in descriptor space, a learned
  predictor, or rejection of the request.
- **Descriptor computation.** What invariants of the
  local environment feed the lookup -- coordination
  number, neighbor identities, neighbor distance
  shells, hashed graph fingerprints, etc.

These questions inherit the schema and lookup
interface from Phase 1, so adding them does not
require a Phase 1 rewrite -- only an enrichment of
the label selection step in 5.6.
