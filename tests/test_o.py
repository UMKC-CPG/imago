...
If IMAGO_DATA is not set, the script looks for <repo_root>/share/ and
...
def main():
    ...
    # Auto-detect IMAGO_DATA when the caller has not set it.
    if not os.environ.get('IMAGO_DATA'):
        candidate = os.path.join(repo_root, 'share')
        if os.path.isdir(candidate):
            os.environ['IMAGO_DATA'] = candidate
            print(f'[test_o] IMAGO_DATA not set; using {candidate}')
        else:
            print(
                '[test_o] Warning: IMAGO_DATA is not set and '
