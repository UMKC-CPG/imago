...
def get_proj_dir(skeleton, custom, kpoints):
    ...
    if (skeleton == "custom"):
        ...
        skeleton_source = os.getenv('IMAGO_DIR') + "/skl/" + skeleton
...
def make_input(skeleton, custom, kpoints):
    ...
    if (skeleton == "custom"):
        with open("imago.skl", "w") as o:
            o.write(custom)
    else:
        skeleton_source = os.getenv('IMAGO_DIR') + "/skl/" + skeleton
        os.system("cp " + skeleton_source + "  ./imago.skl")
