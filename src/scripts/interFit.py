...
- **Output (write_imago_atom_type)**: Already writes s and p counts separately
...
class ScriptSettings():
    ...
    def __init__(self):
        ...
        $IMAGO_RC/interFitrc.py or from the current working directory
        ...
        if not rc_dir:
            sys.exit("Error: $IMAGO_RC is not set. See instructions.")
    ...
    def parse_command_line(self):
        ...
        description_text = """
Gaussian fitting of Grasp2K relativistic orbitals for Imago.
...
Defaults are given in ./interFitrc.py or $IMAGO_RC/interFitrc.py.
"""
...
def get_max_alpha(Z):
  ...
  This table is specific to Grasp2K → Imago relativistic fitting and differs
...
class orbital_fitting:
  """Gaussian fitting of Grasp2K relativistic orbitals to Imago basis functions.
  ...
  def _parse_orbital_info(self):
    ...
    the imago.dat orbital ordering used by the contract program.  Each
  ...
  def write_imago_atom_type(self, f):
    """Write the imago atom type section to a file object.
  ...
  def write_imago_pot_type(self, f):
    """Write the imago potential type section to a file object.
    ...
    (standard imago convention).  Potential alpha range and count come from
  ...
  def write_imago_snippet(self, filename=None):
    ...
    Imago's atomicTypes.f90 and potTypes.f90.
    ...
    if filename:
      with open(filename, 'w') as f:
        self.write_imago_atom_type(f)
        self.write_imago_pot_type(f)
      print(f"  Imago snippet written to {filename}")
    else:
      self.write_imago_atom_type(sys.stdout)
      self.write_imago_pot_type(sys.stdout)
...
def main():
    ...
    element_system.write_imago_snippet('waveFn.dat')
