...
"""element_data.py -- Periodic table and Imago basis/potential parameters.

This module is the Python equivalent of the Perl ElementData.pm module.  It
provides a single class, ElementData, that reads the imago elements database
file ($IMAGO_DATA/elements.dat) and exposes all of its data through a clean,
...
TYPICAL USAGE::
    ...
    ed = ElementData()
    ed.init_element_data()          # reads $IMAGO_DATA/elements.dat
...
INDEXING CONVENTION:
    ...
    throughout the Imago Fortran source and the original Perl scripts, and
...
class ElementData:
    """Stores periodic table data and Imago basis/potential parameters.

    All data are read from the plain-text file $IMAGO_DATA/elements.dat.
    ...
    def init_element_data(self):
        """Read all element data from $IMAGO_DATA/elements.dat.
        ...
        ------
        KeyError
            If the IMAGO_DATA environment variable is not set.
        FileNotFoundError
            If elements.dat cannot be found at $IMAGO_DATA/elements.dat.
        ValueError
    ...
    def apply_bond_factor(self, bonding_factor):
        ...
        throughout Imago.  A factor greater than 1.0 allows longer bonds
