...
imago.skl file and produces the full set of input files needed for any
Imago calculation (SCF, band, DOS, bond, optical, XANES, etc.).

The script follows the XYZ.py / XYZrc.py pattern:
  1. Load defaults from makeinputrc.py ($IMAGO_RC or cwd).
...
(3) You will assign the types depending on the kind of system you are trying
    ...
    a. Is the system crystalline?  If so, types will likely be assigned by
       crystallographic equivalency within the imago.skl input file.  This
    ...
    b. Is the system crystalline with a small perturbation?  (e.g. atom
       ...
       a two step process.  In the first step the crystalline imago.skl file
       ...
       file: ./inputs/imago.mi (makeinput) which contains all atoms
       explicitly listed.  You can apply your perturbation(s) and then copy
       the ./inputs/imago.mi file back out as the new input file.  The
...
(4) What follows is a short review of the different grouping techniques.
    ...
    defined by the imago.skl file and then further modified by xanes
    requests or other targeted requests or similarity classifications etc.
    a. Atoms are initially explicitly grouped in the imago.skl file.
...
class ScriptSettings:
    ...
    def _load_rc():
        ...
        Search order: cwd, then $IMAGO_RC.
        """
    ...
    def assign_rc_defaults(self, rc):
        ...
        # Density-based k-point mode.  When use_kp_density
        # is True, style-code-2 k-point files are written
        # (density + shift + point ops).  When False, style-
        # code-1 files are written (axial counts + shift +
        # point ops).  In both cases Imago builds the mesh
        # internally.  Density values stored per group
        # (1=SCF, 2=PSCF).
    ...
    def _build_parser(self):
        ...
        description_text = """\
Generate all Imago input files from an imago.skl skeleton file.

Creates imago.dat, structure.dat, scfV.dat, k-point files, basis/potential
...
-basisdb    Supply the location of an alternate atomic basis functions
            database.  The default value is $IMAGO_DATA/atomicBDB.

-potdb      Supply the location of an alternate potential function database.
            The default value is $IMAGO_DATA/atomicPDB.
...
-scfkpd     Specify the k-point volume density for the SCF
            ...
            lets Imago compute the per-axis mesh counts from
            the reciprocal cell geometry at runtime.
...
-scfkpint   Select the k-point integration method for the SCF
            ...
            of the k-point input file.  Works with both mesh mode
            (-kp) and density mode (-kpd).
...
-sybdpath   Specify the type of cell and particular path to be used in the
            ...
            options are present in the $IMAGO_DATA/sybdDB directory.  Simply
            specify one of those names as the sybdPath.

-rel        Prepare the Imago input files for a relativistic calculation.
...
"""
    ...
    def _add_parser_arguments(self, parser):
        ...
        # Imago distributes them as uniformly as possible
        # across the three axes at runtime.
        parser.add_argument(
        ...
        # See $IMAGO_DIR/share/xc_code.dat for the list of codes.
...
# File names for Imago input generation.
IMAGO_SKL      = "imago.skl"
IMAGO_FMI      = "imago.fract-mi"
IMAGO_CMI      = "imago.cart-mi"
IMAGO_DAT      = "imago.dat"
POTENTIAL      = "scfV.dat"
KP_SCF         = "kp-scf.dat"
KP_PSCF        = "kp-pscf.dat"
...
# Size of one Imago double-precision number in bytes (set by IMAGO_DOUBLE
# environment variable, default 16 = Fortran COMPLEX*16).
IMAGO_DOUBLE   = int(os.getenv("IMAGO_DOUBLE", "16"))
...
def remove_old_input():
    ...
    for f in [IMAGO_DAT, STRUCTURE, KP_SCF, KP_PSCF]:
        if os.path.exists(f):
            os.remove(f)
...
def setup_environment(settings):
    ...
    if not hasattr(settings, "atomic_bdb"):
        settings.atomic_bdb = os.path.join(imago_data, "atomicBDB")
    if not hasattr(settings, "atomic_pdb"):
        settings.atomic_pdb = os.path.join(imago_data, "atomicPDB")
    settings.sybd_db = os.path.join(imago_data, "sybdDB")
    settings.space_db = os.path.join(imago_data, "spaceDB")
...
def initialize_cell(settings, sc):
    """Read imago.skl, apply space group and supercell, init species.

    This subroutine reads the imago.skl skeleton input file and initialises
    ...
    # Read imago.skl and request to use the species defined in that file.
    ...
    sc.read_input_file(IMAGO_SKL, use_file_species=True)
...
def group_reduce(settings, sc, reduce_idx):
    ...
    **Phase 2 — Compare and assign species.**
    ...
    # The matcher protocol lives in the neutral matchers module
...
def initialize_emu(settings, sc):
    ...
    **What this function does, step by step:**
    ...
    1. **Wipe the project directory.**  All files produced by prior makeinput
       steps (structure.dat, imago.dat, kp files, potentials, etc.) are
       deleted.  Only the original skeleton file ``imago.skl`` is preserved:
    ...
    # --- Step 1: Wipe the project directory, preserving only imago.skl ---
    #
    # The Perl original did:
    #   chdir "..";
    #   system("mv $proj_home/imago.skl emu_temp");
    #   system("rm -r $proj_home/*");
    #   system("mv emu_temp $proj_home/imago.skl");
    #   chdir "$proj_home";
    #
...
def print_imago(settings, sc):
    """Write all Imago output files (imago.dat, structure.dat, scfV, kp, etc.).
    ...
    input file that the Imago Fortran programs need:

    * ``structure.dat`` — lattice vectors, atomic positions, and type IDs
    * ``imago.dat``     — the main control file for every Imago calculation
    ...
    * Job submission scripts (PBS / LSF / Slurm / bash)
    ...
    5. Write ``imago.dat`` (basis sets, potentials, control parameters).
    6. Copy k-point files into the ``inputs/`` directory.
    7. Compute and write memory estimates for each Imago phase.
    8. (Non-XANES only) Write ``imago.fract-mi`` and ``imago.cart-mi``.
    ...
    # The Imago Fortran programs expect Bohr, not Angstroms.
    ...
    for h in range(0, num_xanes_atoms + 1):
        ...
        # 5e. Write imago.dat (the main control file).
        (num_total_core_states, num_total_vale_states,
         num_core_xanes, xanes_control,
         max_num_atom_alphas, pot_dim, nuc_charge) = _print_imago_input(
        ...
        # 5h. (Non-XANES only) Write imago.mi skeleton files.
        if h == 0:
            _make_imago_mi(settings, sc, sorted_elem_id, sorted_spec_id,
        ...
        # We save it on settings so it persists after print_imago returns.
...
def _make_kp(settings, sc):
    ...
    Two modes are supported.  In both modes, Imago builds
    the full k-point mesh internally and performs IBZ
    symmetry reduction using embedded point group operations.
    This gives Imago complete knowledge of the mesh topology
    and symmetry maps, which is required for correct
    computation of decomposition properties (effective
    charge, bond order, PDOS).

    **Mesh mode** (default, ``use_kp_density`` is False):
      Writes a style-code-1 k-point file for each group
      (SCF and post-SCF) containing the per-axis k-point
      counts, a shift, and the point group operations.
      Imago reads the axial counts and builds the mesh.

    **Density mode** (``use_kp_density`` is True):
      Writes a style-code-2 k-point file for each group
      containing a volume density, a shift, and the point
      group operations.  Imago computes the per-axis mesh
      counts from the density and reciprocal cell geometry.
...
def _extract_point_ops(settings):
    """Extract point group operations from the space
    group database.
    ...
    The first ``numSpaceOps / numShifts`` operations
    are the pure point group operations (no centering
    translations).  We extract only these.  Each
    operation consists of a 3x3 rotation matrix and a
    fractional translation vector.  For symmorphic
    space groups the translations are all zero; for
    non-symmorphic groups (e.g. Fd-3m) some operations
    carry non-zero translations (screw axes, glide
    planes).

    Parameters
    ----------
    settings : ScriptSettings
        Must have ``space_db`` and
        ``space_group_name`` set.

    Returns
    -------
    tuple of (list, list)
        ``point_ops`` — each element is a 3x3 matrix
...
def _write_mesh_kp_file(dest_path, kp_mesh,
    ...
    This file is read by ``readKPoints`` in
    ``kpoints.f90``.  It tells Imago to build the
    k-point mesh internally from explicit per-axis
    counts, then perform IBZ symmetry reduction using
    the embedded point group operations.

    This replaces the legacy workflow that called the
    external ``makeKPoints`` program to produce a
    style-code-0 explicit k-point list.  By writing
    style code 1, Imago builds the full mesh
    internally and has complete knowledge of the mesh
    topology and symmetry maps needed for correct
    decomposition properties (effective charge, bond
    order, PDOS).

    The file format matches the style-code-1 branch
    of ``readKPoints``:
      - ``KPOINT_STYLE_CODE`` = 1
      - ``KPOINT_INTG_CODE``  = integration method
        (0 = Gaussian/histogram, 1 = LAT, etc.)
      - ``NUM_KP_A_B_C`` = per-axis k-point counts
...
def _write_density_kp_file(dest_path, density,
    ...
    This file is read by ``readKPoints`` in
    ``kpoints.f90``.  It tells Imago to compute the
    per-axis k-point mesh internally using
    ``computeAxialKPoints``, based on the minimum
    k-point volume density and the reciprocal cell
    geometry.  It also embeds the point group
    operations so that Imago can perform IBZ symmetry
    reduction at runtime.

    The file format matches the style-code-2 branch
    of ``readKPoints``:
      - ``KPOINT_STYLE_CODE`` = 2
      - ``KPOINT_INTG_CODE``  = integration method
        (0 = Gaussian/histogram, 1 = LAT, etc.)
      - ``MIN_KP_LINE_DENSITY`` = the volume density
        (label is historical; the value is kpoints
        per unit reciprocal-space volume in Bohr^-3)
      - ``KP_SHIFT_A_B_C`` = the a, b, c shift values
      - ``NUM_POINT_OPS`` = number of point group ops
      - ``POINT_OPS`` label, then for each operation a
        3x3 rotation matrix (3 lines) followed by a
    ...
    dest_path : str
        ...
        ``.inputTemp/kp-scf.dat``).
    density : float
        The minimum k-point volume density (kpoints
        per unit reciprocal-space volume, Bohr^-3).
    kp_shift : str
        Space-separated shift values
        (e.g. ``"0.5 0.5 0.5"``).
    point_ops : list of list[list[float]]
...
def _contract_basis(settings, sc):
    ...
    referenced later when writing ``imago.dat``.
...
def _get_cumulative_types(settings, file_set):
    ...
    The Imago programs use a single sequential numbering for types across
...
def _sort_atoms(settings, sc, file_set, cumulative_num_types):
    ...
    Atoms must be ordered by element → species → type for the imago
...
def _print_structure(settings, sc, file_set, sorted_elem_id, sorted_spec_id,
    ...
    This file tells the Imago Fortran programs where the atoms are and
    ...
    * ``NUM_POTENTIAL_SITES`` — same block repeated for potential sites
      (in Imago the atom sites and potential sites are identical).
    ...
    with open(path, "w") as f:
        ...
        # Potential sites (identical to atom sites in Imago).
...
def _print_imago_input(settings, sc, file_set,
                       sorted_elem_id, sorted_spec_id, sorted_type_id,
                       sorted_xanes_atoms, cumulative_num_types,
                       total_num_types, num_atoms_of_type):
    """Write ``imago.dat`` — the main Imago control file.

    This is the largest output file produced by makeinput.  It contains
    everything the Imago programs need beyond the structure and k-points:
    ...
    * END_OF_DATA marker
    ...
    imago_path = os.path.join(INPUTS_DIR, IMAGO_DAT)
    imago_fh = open(imago_path, "w")

    # Title.
    imago_fh.write("TITLE\n")
    title_str = " ".join(sc.system_title[1:])
    imago_fh.write(f"{title_str}\n")
    imago_fh.write("END_TITLE\n")

    # Number of atom types.
    imago_fh.write("NUM_ATOM_TYPES\n")
    imago_fh.write(f"{total_num_types:<5d}\n")

    # Write each type's basis set and extract state count information.
    (num_total_core_states, num_total_vale_states,
     num_core_xanes, xanes_control, max_num_atom_alphas) = \
        _print_basis_set(
            settings, sc, file_set, imago_fh,
            sorted_elem_id, sorted_spec_id, sorted_type_id,
            sorted_xanes_atoms, cumulative_num_types,
            total_num_types, num_atoms_of_type)

    # Number of potential types.
    imago_fh.write("NUM_POTENTIAL_TYPES\n")
    imago_fh.write(f"{total_num_types:<5d}\n")
    ...
    # Exchange correlation mesh.
    imago_fh.write("PRINT_XC_MESH\n")
    imago_fh.write("0\n")
    imago_fh.write("NUM_ANGULAR_SAMPLE_VECTORS\n")
    imago_fh.write(f"{settings.num_samp_vectors}\n")
    imago_fh.write("WTIN_WTOUT\n")
    imago_fh.write(f"{settings.xc_in_weight} {settings.xc_out_weight}\n")
    imago_fh.write("RADIAL_SAMPLE-IN_OUT_SPACING\n")
    imago_fh.write(f"{settings.xc_in_samp} {settings.xc_out_samp}"
    ...
    imago_fh.write("SHARED_INPUT_DATA\n")

    # Space group.
    imago_fh.write("SPACE_GROUP\n")
    imago_fh.write(f"{settings.space_group_num} {settings.space_group_sub_num}\n")

    # Cutoffs.
    imago_fh.write("BASISFUNCTION_AND_ELECTROSTATIC_CUTOFFS\n")
    imago_fh.write(f"    1.00000000E-{settings.bf_cutoff}"
                   f"    1.00000000E-{settings.es_cutoff}\n")

    # States.
    imago_fh.write("NUM_STATES_TO_USE\n")
    imago_fh.write(f"{num_states_used[1]} {num_states_used[2]}"
                   f" {num_states_used[3]}\n")

    # Electrons.
    imago_fh.write("NUM_ELECTRONS\n")
    imago_fh.write(f"{int(num_electrons)}\n")

    # Thermal smearing.
    imago_fh.write("THERMAL_SMEARING_SIGMA__SIGMA_CUTOFF\n")
    imago_fh.write(f"{settings.therm_smear_main} 11.5\n")

    # Fermi level search limit.
    imago_fh.write("FERMI_LEVEL_SEARCH_LIMIT\n")
    imago_fh.write("13.6\n")

    # Main (SCF) input data.
    imago_fh.write("MAIN_INPUT_DATA\n")
    imago_fh.write("LAST_ITERATION\n")
    imago_fh.write(f"{settings.num_iter_main}\n")
    imago_fh.write("CONVERGENCE_TEST\n")
    imago_fh.write(f"{settings.converg_main}\n")
    imago_fh.write("XC_CODE\n")
    imago_fh.write(f"{settings.xc_code}\n")
    imago_fh.write("FEEDBACK_LEVEL\n")
    imago_fh.write("2\n")
    imago_fh.write("RELAXATION_FACTOR\n")
    imago_fh.write("0.2\n")
    imago_fh.write("EACH_ITER_FLAGS__TDOS\n")
    imago_fh.write(f"{settings.iter_tdos}\n")
    imago_fh.write("PLUSUJ_FORM__NUM_ITEMS\n")
    imago_fh.write("0 0      ! 0,1,2: elements, types, atoms; num items\n")
    imago_fh.write("ID__U__J\n")
    imago_fh.write("NUM_SPLIT_TYPES__DEFAULT_SPLIT\n")
    imago_fh.write(f"0 {settings.spin_main}\n")
    imago_fh.write("TYPE_ID__SPIN_SPLIT_FACTOR\n")

    # Dipole input data.
    imago_fh.write("DIPOLE_INPUT_DATA\n")
    imago_fh.write("DIPOLE_CENTER\n")
    imago_fh.write("0.0 0.0 0.0    ! FractABC: center about which"
                   " to compute the dipole.\n")

    # DOS input data.
    imago_fh.write("DOS_INPUT_DATA\n")
    imago_fh.write(f"{settings.e_delta_dos} {settings.sigma_dos}"
                   f"              ! DOS Delta Energy, DOS sigma broadening"
                   f" (FWHM)\n")
    imago_fh.write(f"{settings.emin_dos} {settings.emax_dos}"
                   f"                 ! DOS EMIN and EMAX\n")
    imago_fh.write(f"{settings.detail_code_pdos}"
                   f" ! PDOS ctrl flag: 0 t; 1 a total; 2 a nl; 3 a nlm\n")

    # Bond input data.
    imago_fh.write("BOND_INPUT_DATA\n")
    imago_fh.write(f"{settings.max_len_bond}"
                   f"                     ! MAXIMUM BOND LENGTH\n")
    imago_fh.write(f"{settings.e_delta_bond} {settings.sigma_bond}"
                   f"               ! BOND Delta Energy,"
                   f" BOND sigma broadening\n")
    imago_fh.write(f"{settings.emin_bond} {settings.emax_bond}"
                   f"                  ! BOND EMIN and EMAX\n")
    imago_fh.write(f"{settings.output_bondq}"
                   f"        ! Flag for BONDQ output style 0=new a; 1=old t\n")
    imago_fh.write(f"{settings.bond_3c}"
                   f"                         ! Flag for 3C-BOND\n")
    imago_fh.write(f"{settings.max_neighbor_bond}"
                   f"                     ! Max # neighbor atoms\n")

    # SYBD input data.
    imago_fh.write("SYBD_INPUT_DATA\n")
    _process_sybd_path(settings, sc, imago_fh)

    # MTOP input data.  kp_mesh_pscf is 1-indexed; the three mesh
    # counts live in slots 1..3.
    imago_fh.write("MTOP_INPUT_DATA\n")
    imago_fh.write(f"{settings.kp_mesh_pscf[1]} {settings.kp_mesh_pscf[2]}"
                   f" {settings.kp_mesh_pscf[3]} # PSCF\n")
    imago_fh.write("0.0 0.0 0.0 # Shift\n")

    # PACS (XANES/ELNES) input data.
    imago_fh.write("PACS_INPUT_DATA\n")
    imago_fh.write(f"{sorted_xanes_atoms[file_set]}"
                   f"                       ! Excited atom number\n")
    imago_fh.write(f"{settings.e_delta_pacs} {settings.sigma_pacs}"
                   f"               ! PACS delta Energy,"
                   f" PACS sigma factor\n")
    imago_fh.write(f"{settings.onset_slack_pacs}"
                   f" {settings.energy_window_pacs}"
                   f"               ! Energy slack before onset,"
                   f" energy window\n")
    imago_fh.write(f"{num_core_xanes}"
                   f"                    ! Number of possible core"
                   f" orbitals to excite\n")
    for orbital in range(1, num_core_xanes + 1):
        imago_fh.write(f"{xanes_control[orbital]}"
                       f" -1 ! QN_n QN_l Init1 Init2 TEDiff\n")

    # Optical properties input data.
    imago_fh.write("OPTC_INPUT_DATA\n")
    imago_fh.write(f"{settings.e_cutoff_optc}"
                   f"                    ! OPTC energy cutoff\n")
    imago_fh.write(f"{settings.e_trans_optc}"
                   f"                     ! OPTC energy trans\n")
    imago_fh.write(f"{settings.e_delta_optc}"
                   f"                     ! OPTC delta energy\n")
    ...
    imago_fh.write(f"{settings.e_cutoff_nlop}"
                   f"                    ! NLOP energy cutoff\n")
    imago_fh.write(f"{settings.e_trans_nlop}"
                   f"                     ! NLOP energy trans\n")
    imago_fh.write(f"{settings.e_delta_nlop}"
                   f"                     ! NLOP delta energy\n")
    imago_fh.write(f"{settings.sigma_nlop}"
                   f"                      ! NLOP broadening\n")

    # Sigma(E) input data.
    imago_fh.write("SIGE_INPUT_DATA\n")
    imago_fh.write(f"{settings.e_cutoff_sige}"
                   f"                    ! SIGE energy cutoff\n")
    imago_fh.write(f"{settings.e_trans_sige}"
                   f"                     ! SIGE energy trans\n")
    imago_fh.write(f"{settings.e_delta_sige}"
                   f"                     ! SIGE delta energy\n")
    imago_fh.write(f"{settings.sigma_sige}"
                   f"                      ! SIGE broadening\n")

    # Field input data.
    imago_fh.write("FIELD_INPUT_DATA\n")
    imago_fh.write("10 10 10"
                   "                      ! a,b,c # of mesh points\n")
    imago_fh.write("-100000.0 100000.0"
                   "            ! min,max range of energy\n")
    imago_fh.write("1 1 1 1"
...
def _print_basis_set(settings, sc, file_set, imago_fh,
                     sorted_elem_id, sorted_spec_id, sorted_type_id,
                     sorted_xanes_atoms, cumulative_num_types,
                     total_num_types, num_atoms_of_type):
    """Write the basis set section of imago.dat and extract state counts.
    ...
    Parameters
    ----------
    (see _print_imago_input for parameter descriptions)
    ...
    for element in range(1, settings.num_elements + 1):
        for species in range(1, settings.num_species[element] + 1):
            for type_id in range(1, settings.num_types[file_set][element][species] + 1):
                ...
                # Write the header.
                imago_fh.write("ATOM_TYPE_ID__SEQUENTIAL_NUMBER\n")
                imago_fh.write(f"{element:<5d} {species:<5d} {type_id:<5d}"
                               f"     {seq_type}\n")
                imago_fh.write("ATOM_TYPE_LABEL\n")
                imago_fh.write(f"{type_label}\n")
                ...
                imago_fh.writelines(basis_lines)
...
def _print_scf_pot(settings, sc, file_set, imago_fh,
                   sorted_elem_id, sorted_spec_id, sorted_type_id,
                   sorted_xanes_atoms, cumulative_num_types,
                   total_num_types, num_atoms_of_type):
    """Write the SCF potential section of imago.dat and the scfV.dat file.
    ...
    2. Read the potential function definition from the database file and
       write it into ``imago.dat``, applying any potential modifications
    ...
    for element in range(1, settings.num_elements + 1):
        for species in range(1, settings.num_species[element] + 1):
            for type_id in range(1, settings.num_types[file_set][element][species] + 1):
                ...
                imago_fh.write("POTENTIAL_TYPE_ID__SEQUENTIAL_NUMBER\n")
                imago_fh.write(f"{element:<5d} {species:<5d} {type_id:<5d}"
                               f"     {seq_type}\n")
                imago_fh.write("POTENTIAL_TYPE_LABEL\n")
                imago_fh.write(f"{type_label}\n")
                ...
                imago_fh.write(pot_lines[0])

                # Line 1: nuclear charge line
                imago_fh.write(pot_lines[1])
                ...
                imago_fh.write(pot_lines[2])
                imago_fh.write(pot_lines[3])
                imago_fh.write(pot_lines[4])
                ...
                if (settings.mod_element_name.lower() ==
                    ...
                    imago_fh.write(f"{settings.num_mod_terms}\n")
                    pot_dim += settings.num_mod_terms
                    imago_fh.write("ALPHAS\n")
                    imago_fh.write(f"{settings.min_mod_term:14.8e}"
                                   f"  {settings.max_mod_term:14.8e}\n")
                else:
                    imago_fh.write(pot_lines[5])
                    nt_vals = pot_lines[5].split()
                    pot_dim += int(nt_vals[0])
                    imago_fh.write(pot_lines[6])
                    imago_fh.write(pot_lines[7])
...
def _process_sybd_path(settings, sc, imago_fh):
    ...
    8. Write everything to imago.dat.
    ...
    settings : ScriptSettings
    sc : StructureControl
    imago_fh : file handle
        The open imago.dat file.
    """
...
def _make_imago_mi(settings, sc, sorted_elem_id, sorted_spec_id,
                   sorted_fract_abc, sorted_direct_xyz, output_file):
    """Write an imago.mi skeleton file with explicit species assignments.
    ...
    * ``imago.fract-mi`` — atomic positions in fractional coordinates
    * ``imago.cart-mi``  — atomic positions in Cartesian coordinates (Å)
    ...
    with open(path, "w") as f:
        f.write("title\n")
        f.write("Imago file created by makeinput\n")
        title_str = " ".join(sc.system_title[1:])
        f.write(f"{title_str}\n")
        f.write("end\n")
        f.write("cell\n")
...
def _compute_and_print_mem(settings, sc, file_set,
                           ...
                           total_num_types, num_atoms_of_type,
                           max_num_atom_alphas, pot_dim, nuc_charge):
    """Compute memory estimates for each Imago phase and write them.

    For each of the three basis set sizes (minimal, full, extended), the
    memory usage is estimated for every Imago program phase:
    ...
    * **OPTC**: Transition pairs + momentum matrices + wave functions.
    ...
    (×IMAGO_DOUBLE) and express in megabytes or gigabytes depending on
    magnitude.
    ...
    max_est_bytes = max_est * IMAGO_DOUBLE
    ...
    for phase in all_phases:
        for bt in range(1, 4):
            phase[bt] *= IMAGO_DOUBLE / memory_factor
...
def _make_sub_file(settings, h, current_name, sub_dir, proj_home,
    ...
    * Source of the imago environment.
    * The appropriate Imago commands for this file set.
...
def print_summary(settings, sc):
    """Write a concise summary of the generated Imago input.
    ...
    * **Matrix dimensions** — the number of valence states and core states
      ...
      matrices that Imago will allocate and thus the memory and CPU cost.
    ...
    * **XANES details** (if applicable) — for each target atom, the
    ...
    with open(SUMMARY_FILE, "w") as f:
        ...
        # K-point information.  In both modes (mesh and
        # density), the IBZ-reduced k-point count is
        # computed by Imago at runtime and is not known
        # here.  We print the mesh specification instead.
