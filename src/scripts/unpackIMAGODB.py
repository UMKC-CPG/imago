...
"""PROGRAM: unpackIMAGODB.py

PURPOSE: This program unpacks the database files that come with the
   Imago package. The databases include atomic basis sets, atomic
   ...
   to by the $IMAGO_DATA environment variable and must be
   unpacked before the imago package can be used.
...
USAGE: unpackIMAGODB.py [-h]

   This script takes no arguments beyond the optional -h for
   help. It relies entirely on the $IMAGO_DATA environment
...
class ScriptSettings():
    """The instance variables of this object are the user settings
       ...
       a consistent structure with other imago Python scripts and
    ...
    def parse_command_line(self):
        ...
        prog_name = "unpackIMAGODB.py"

        description_text = """
Version: 1.0 (March 2026)
Requires: $IMAGO_DATA environment variable set to the imago
          data directory containing the .tgz database files.

Unpack the imago database files (.tgz archives) located in the
$IMAGO_DATA directory. This must be done once after installing
or updating the imago package before any calculations can be
...
def unpack_databases():
    """Unpack all Imago database tarballs in the $IMAGO_DATA
    directory and rebuild the space group database.

    The function changes to the $IMAGO_DATA directory, extracts
    ...
    # Determine the imago data directory from the environment.
    imago_data = os.getenv('IMAGO_DATA')
    if not imago_data:
        sys.exit(
            "Error: $IMAGO_DATA is not set. "
            "Please set it to the imago data directory."
        )

    # Go to the $IMAGO_DATA directory (which is where the
    #   databases are).
    try:
        os.chdir(imago_data)
    except OSError as e:
        sys.exit(
            f"Error: Cannot change to $IMAGO_DATA "
            f"directory '{imago_data}': {e}"
        )
    ...
    # Return to the original working directory (the imago
    #   data directory).
    os.chdir("..")
...
def main():
    ...
    # Unpack the imago databases.
    unpack_databases()

    # Report success.
    print("Imago databases unpacked successfully.")
