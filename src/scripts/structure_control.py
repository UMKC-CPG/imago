...
It is the central library for all Imago pre- and post-processing scripts.
...
TYPICAL USAGE::
    ...
    sc = StructureControl()
    sc.read_imago_skl()           # read imago.skl in the current directory
    sc.assign_coval_radii()       # pull covalent radii from ElementData
    sc.create_bonding_list()      # compute bonds based on covalent radii
    sc.print_imago()              # write the structure back out

ROLE IN THE IMAGO WORKFLOW:
    Scripts that use StructureControl fall into three categories:
    1. Input preparation   -- convert external formats to imago.skl
    2. Execution control   -- set up and launch Imago Fortran calculations
...
INDEXING CONVENTION:
    ...
    Imago Fortran source and the original Perl code.  Index 0 of every such
...
class StructureControl:
    """Atomic structure container and manipulation library for Imago.
    ...
    ~~~~~~~~~~~~~~~~~
    imago_skl : str
        Path to the imago skeleton input file (default: 'imago.skl').
    model_xyz : str
    ...
    space_group_db : str
        Path to the space group database ($IMAGO_DATA/spaceDB).
    atomic_bdb : str
        Path to the atomic basis set database ($IMAGO_DATA/atomicBDB).
    atomic_pdb : str
        Path to the atomic potential database ($IMAGO_DATA/atomicPDB).
    title : str
    ...
    def _init_state(self):
        ...
        # Imago Fortran code.  Index 0 of every list is a None placeholder.
        #
    ...
    def reset(self):
        ...
        default file names (imago.skl, model.xyz, …), database paths, and
    ...
    def read_imago_skl(self, filename=None, use_file_species=True,
        ...
        ``space <group>``
            ...
            space-group database directory ($IMAGO_DATA/spaceDB).  The
        ...
        filename : str, optional
            Path to the skeleton file.  Defaults to self.imago_skl
            ('imago.skl' in the current directory).
        ...
        # symlink) in the space-group database (usually $IMAGO_DATA/spaceDB).
    ...
    def read_imago_species(self, filename=None):
        """Read species assignments from inputs/imago.fract-mi.

        This file is generated by the ``makeinput`` script and records the
        imago atom positions in fractional coordinates together with the
        ...
        ----------
        filename : str, optional
            Path to the fract-mi file. Defaults to 'inputs/imago.fract-mi'.
        """
        if filename is None:
            filename = 'inputs/imago.fract-mi'
    ...
    def read_input_file(self, filename=None, use_file_species=True):
        ...
        The dispatch order matters: 'imago' and 'skl' both map to the imago
        ...
        if 'imago' in fname or 'skl' in fname:
            self.read_imago_skl(filename, use_file_species)
        elif 'pdb' in fname:
    ...
    def read_pdb(self, filename=None, use_file_species=True):
        ...
        3×3 real-space lattice vector representation used throughout Imago.
        ...
        Species assignment strategy:
          ...
          the element+IDnumber atomTag used elsewhere in Imago — it is never a
        ...
        # which is NOT the element+species-number combination used by Imago.
        pdb_atom_tag = [None]
        ...
        with open(filename, 'r') as fh:
            for raw in fh:
                ...
                if 'TER' in record:
                    ...
                    # the degree form (used by print_imago and other output
    ...
    def read_struct(self, filename=None, use_file_species=True):
        """Read an Imago internal struct file (CELL_VECTORS / NUM_ATOM_SITES format).
    ...
    def read_abc(self, filename=None, use_file_species=True):
        ...
        The abc format is the simplest of the imago readers: no title line,
        ...
        imago.skl.  The bare element name is recovered by stripping any
    ...
    def read_cif(self, filename=None, use_file_species=True):
        ...
        .. note::
            ...
            in the format created by the imago ``makeinput`` program.  Perhaps
        ...
        first ``_`` is extracted and lowercased to form the imago atom tag
        ...
        with open(filename, 'r') as fh:
            ...
            for raw in fh:
                ...
                # imago atom tag (element symbol + optional species digit).
    ...
    def read_lmp(self, filename=None, use_file_species=True):
        ...
        This reads the LAMMPS *data* file format written by Imago's
        ...
        only read the title line).  The actual Imago–LAMMPS interchange
        ...
        **Box-bounds convention**: printLMP writes the lattice *magnitudes*
        ...
        the same way, so the imago round-trip is self-consistent, but the
        ...
        **Masses section**: one entry per unique Imago (element, species)
        ...
        file not produced by Imago), elem is set to 'x' and species to 1.
        ...
        **Perl bug note**: printLMP's Atoms section used ``$currentSpecies``
        ...
        #   b = (xy, ly,  0)
        #   c = (xz, yz, lz)
        # This is identical to the triclinic P1 convention used in Imago.
        # ------------------------------------------------------------------
    ...
    def read_bond_analysis_bl(self, filename=None):
        ...
        been previously loaded (e.g. no associated imago.skl file is required).
    ...
    def read_bond_analysis_ba(self, filename=None):
        ...
        independently of any imago.skl file.  self.num_atoms must already
    ...
    def print_imago(self, filename=None, title='', style='frac'):
        """Write the current structure in imago skeleton (.skl) format.
        ...
        ----------
        filename : str, optional
            Output path.  Defaults to self.imago_skl.
        title : str, optional
    ...
    def print_imago_map(self, filename=None):
        """Write the dat-to-skl atom index map file.

        This file records the identity of every atom in a fully-expanded Imago
        ...
        ``%6d %7d %7d %10s %10s %7s``::
            ...
            OATOM#  : (int, w=6)  imago atom index, 1..num_atoms
            ...
            TYPE    : (str, w=7)  imago atom tag, i.e. the species identifier
                                  used in the .skl file (e.g. 'si_tb2')

        Parameters
        ----------
        filename : str, optional
            Output path.  Defaults to ``'imago.map'``.
        """
        if filename is None:
            filename = 'imago.map'
    ...
    def print_vasp(self, filename='POSCAR'):
        ...
        POSCAR format
        -------------
        The POSCAR is written entirely in Imago's internal Bohr units with a
        ...
        mean fractional coordinates, which is the opposite of Imago's
        convention.  The mapping is:

        - VASP ``direct``    == Imago ``fract``  (fractions of lattice vectors)
        - VASP ``Cartesian`` == Imago ``direct``  (Cartesian/XYZ in Bohr)
        ...
        with open(filename, 'w') as f:
            ...
            f.write(' '.join(counts) + '\n')
            # "direct" is VASP's word for fractional coordinates (== Imago
            # "fract").  Do not confuse with Imago "direct" (== VASP
            # "Cartesian").
            f.write('direct\n')
            for atom in range(1, self.num_atoms + 1):
    ...
    def print_pdb(self, filename=None):
        ...
        Imago use-case where cell parameters are always defined.

        CRYST1 record layout (values in Bohr — Imago internal units):
          cols  1- 6   "CRYST1"
          cols  7-15   a (9.3f)
          cols 16-24   b (9.3f)
          cols 25-33   c (9.3f)
        ...
        Coordinate units: Imago stores atom positions in Bohr internally.
    ...
    def print_cif(self, filename=None):
        ...
        ----------------------
        Imago always writes fully-expanded P1 structures — no symmetry
        ...
        Cell parameters
        ---------------
        Imago stores lattice magnitudes internally in Bohr radii.  CIF requires
        ...
        # CIF requires Angstroms; Imago stores magnitudes in Bohr internally.
        ...
        with open(filename, 'w') as f:
            ...
            # Always P 1 (triclinic) — Imago writes fully-expanded structures.
    ...
    def print_lmp(self, filename=None):
        ...
        type IDs to be contiguous — which is guaranteed by the imago atom
    ...
    def reorder_atom_coordinates(self):
        ...
        the read_imago_skl call chain.  The Python port loops over all atoms
    ...
    def create_element_list(self):
        ...
        Called by read_imago_skl() immediately after the atom list is read,
    ...
    def create_species_data(self, use_file_species=True):
        ...
        Called by read_imago_skl() immediately after create_element_list().
        """
    ...
    def compute_implicit_info(self):
        ...
        to establish four global extrema used downstream by the imago program:
    ...
    def create_coordination_summary(self):
        ...
        # the None placeholder (1-indexed convention used throughout Imago).
