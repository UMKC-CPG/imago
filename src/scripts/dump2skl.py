...
PURPOSE:  This program takes a LAMMPS xyz coordinate dump file as
   input and writes an imago skeleton file (imago.skl) of a
   specified timestep.
UPDATED:  Mar. 20, 2026
USAGE:    This program takes a LAMMPS dump file and an associated
   ...
   writes a file called imago.skl.
...
class ScriptSettings():
    ...
    def __init__(self):
        ...
        location: $IMAGO_RC/dump2sklrc.py or from the current
    ...
    def parse_command_line(self):
        ...
        description_text = """\
Convert a LAMMPS dump file to an imago skeleton (.skl) file.
...
against the imago element database, and writes an imago.skl
file suitable for use with the imago code.
...
USAGE NOTES:
  ...
  - Timestep numbers must match an "ITEM: TIMESTEP" value that
    actually appears in the dump file."""
        ...
        epilog_text = """\
Defaults are given in ./dump2sklrc.py or $IMAGO_RC/dump2sklrc.py.
"""
...
def get_lammps_types(data_file):
    ...
    atomic masses of all elements in the imago element database.
...
def write_skl(name, timestep, cell_dims, angles,
              ...
              elements):
    """Write the imago skeleton (.skl) file.

    The skeleton file is the primary input geometry file for
    the imago code.  It contains:
    ...
    The file is written as 'imago.skl' in the current
    directory.
...
def main():
    ...
    Flow:
      ...
      5. Read the box geometry and atom coordinates for the
         selected frame.
      6. Process coordinates and write the imago.skl file.
    """
    ...
    # the data file to the known masses in the imago element
    # database.
