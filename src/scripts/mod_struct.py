...
an imago structure file (imago.skl by default) and applies a
...
The script follows the XYZ.py / XYZrc.py pattern:
  1. Load defaults from mod_structrc.py ($IMAGO_RC or cwd).
...
-i INFILE
    Alternate input file (default: imago.skl).

-o OUTFILE
    Alternate output file (default: imago.skl.new).
...
class ScriptSettings:
    ...
    def _load_rc():
        ...
        Search order: cwd first (local override), then $IMAGO_RC.
        """
        ...
        if os.path.isfile(cwd_rc):
            ...
            if not rc_dir:
                sys.exit(
                    "Error: $IMAGO_RC is not set and no local "
...
perturbation, surface preparation) to an imago structure file.
...
"""
        ...
        epilog_text = """\
Defaults are given in ./mod_structrc.py or $IMAGO_RC/mod_structrc.py.
"""
