...
class ScriptSettings():
    ...
    def __init__(self):
        ...
        location: $IMAGO_RC/makePotDBrc.py or from the current
...
def get_database_paths():
    """Determine the atomic database paths from $IMAGO_DATA.
    ...
    if not data_dir:
        sys.exit(
            "Error: $IMAGO_DATA is not set. See instructions."
        )
...
def _fit_one_element(element, atomic_pdb, atomic_bdb):
    ...
    #   polarization. These values are read by Imago, but not
    #   used.
