...
PURPOSE:  This program will run an isolated atom calculation
          ...
          element), the script:
            ...
            2. Writes a minimal imago.skl file with a single
               atom in a 20x20x20 Angstrom cell.
            3. Runs makeinput to generate Imago input files.
            4. Runs imago to perform the SCF calculation.
          After all elements are computed, the script:
            ...
            7. Copies the existing potential definition file
               (pot1) from $IMAGO_DATA/atomicPDB/.
...
class ScriptSettings():
    ...
    $IMAGO_RC (or from the current working directory if a
...
Defaults: ./runIsoAtomsrc.py or $IMAGO_RC/runIsoAtomsrc.py.
"""
...
def make_atom_pot(settings, element_names, num_elements):
    ...
    For each element in the specified range, this function:
      1. Creates a directory named after the element.
      2. Writes a minimal imago.skl file containing a single
         atom of that element placed at the origin of a
         20x20x20 Angstrom cubic cell with 90-degree angles.
      3. Runs makeinput to generate the Imago input files.
         If --nocore was requested, the -nocore flag is
         passed to makeinput.
      4. Runs imago (the unified imago SCF solver) to
    ...
    for element in range(element_init, element_fin + 1):
        ...
        # Run makeinput to generate Imago input files.
        ...
        # Run imago to perform the SCF calculation.
        imago_path = os.path.join(imago_bin, "imago")
        subprocess.run(
            imago_path, shell=True, check=True
        )
...
def make_pdb(settings, element_names, num_elements):
    ...
    For each element in the specified range, this function:
      ...
      4. Copies the remaining potential coefficients into
         two files:
           - coeff1: the standard coefficient file used by
             Imago for subsequent calculations.
      ...
      5. Copies the existing potential definition file
         (pot1) from $IMAGO_DATA/atomicPDB/<element>/pot1
