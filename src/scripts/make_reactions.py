...
ALGORITHM:
   ...
   new directory, it will copy imago skeleton input files from the
   ...
   created, the imago skeleton file containing the merged pair of
...
REQUIREMENTS:
   ...
   - angle_utils.py (cluster_angles helper for angle-type
     discovery; replaces the legacy $IMAGO_DATA/angles.dat
     lookup -- see DESIGN 4.8 for the geometry-derived
     clustering approach)
   - $IMAGO_RC/make_reactionsrc.py (default parameters)
...
class ScriptSettings:
    ...
    ($IMAGO_RC/make_reactionsrc.py or ./make_reactionsrc.py) and
    ...
    def __init__(self):
        ...
        $IMAGO_RC/make_reactionsrc.py or from the current working
        directory if a local copy is present."""

        # Read default variables from the resource control file.
        rc_dir = os.environ.get('IMAGO_RC')
        if not rc_dir:
            sys.exit("Error: $IMAGO_RC is not set. "
...
an imago skeleton (.skl) file. The templates are suitable for use
...
class MakeReactions:
    ...
    def run(self):
        ...
        #   imago.skl files. There is one phase to a complete
    ...
    def prep_molecules(self):
        ...
        # Read the first imago skeleton file and then compute
    ...
    def catalog_s(
        ...
        """
        # Read the centered imago skeleton file and extract
        #   key data from it.
    ...
    def catalog_elem_spec_mol_id(self):
        ...
        for s1 in range(1, self.num_s_atoms1 + 1):
            for s2 in range(1, self.num_s_atoms2 + 1):
                ...
                merged_file = f"imago_{s1}_{s2}.skl"
                sc = self.StructureControl()
                sc.read_input_file(
    ...
    def prune_merged_molecule(self):
        ...
        for s1 in range(1, self.num_s_atoms1 + 1):
            for s2 in range(1, self.num_s_atoms2 + 1):
                ...
                merged_mol_file = f"imago_{s1}_{s2}.skl"
                subprocess.run(
                    [
                        "bond_analysis.py",
                        "-i",
    ...
    def make_reaction_templates(self, rxn_phase):
        ...
        for s1 in range(1, self.num_s_atoms1 + 1):
            for s2 in range(1, self.num_s_atoms2 + 1):
                ...
                pruned_file = f"imago_{s1}_{s2}.skl"
                sc = self.StructureControl()
                sc.read_input_file(
