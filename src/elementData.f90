module O_ElementData
   ...
   integer :: numUniqueElements ! Number of elements with some physical data
         ! from the periodic table of the elements that are included in Imago.

   contains

! This subroutine will read a bunch of data from the elements.dat file. Much
!   of the data in elements.dat is already known by the imago program upon
!   reading the input file or it is not specifically needed by the imago
...
subroutine initElementData
   ...
   call get_environment_variable(NAME="IMAGO_DATA",VALUE=dataDirectory,&
         & STATUS=info)
   if (info /= 0) then
      write (20,*) "info=",info
      stop "Failed to get IMAGO_DATA environment variable."
   endif
