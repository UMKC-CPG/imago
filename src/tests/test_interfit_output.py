...
write_imago_snippet methods by constructing mock orbital_fitting objects
...
def _make_mock_fitter(Z, names, l_list, lvals, max_l, num_terms, alphas,
    ...
    _Fitter.write_imago_atom_type = interFit.orbital_fitting.write_imago_atom_type
    _Fitter.write_imago_pot_type = interFit.orbital_fitting.write_imago_pot_type
    _Fitter.write_imago_snippet = interFit.orbital_fitting.write_imago_snippet
...
class TestAtomTypeOutput:
    ...
    def test_tags_present(self, carbon_mb):
        buf = io.StringIO()
        carbon_mb.write_imago_atom_type(buf)
        text = buf.getvalue()
    ...
    def test_alpha_counts_monotonic(self, aluminium_fb):
        ...
        buf = io.StringIO()
        aluminium_fb.write_imago_atom_type(buf)
...
class TestPotTypeOutput:
    ...
    def test_tags_present(self, carbon_mb):
        buf = io.StringIO()
        carbon_mb.write_imago_pot_type(buf)
        text = buf.getvalue()
...
class TestSnippetOutput:
    ...
    def test_snippet_contains_both_sections(self, carbon_mb, tmp_path):
        ...
        carbon_mb.write_imago_snippet(outfile)
    ...
    def test_snippet_raises_without_fitting(self):
        ...
        _Empty.write_imago_snippet = interFit.orbital_fitting.write_imago_snippet

        obj = _Empty()
        with pytest.raises(RuntimeError, match="No fitting results"):
            obj.write_imago_snippet('test.dat')
