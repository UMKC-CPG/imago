...
database (IMAGO_DATA/elements.dat) needs to be accessible.
...
class TestDotProduct:
    ...
    Both vectors are 1-indexed ``[None, x, y, z]`` matching the
    module-wide convention (Perl's ``@v[1..3]``).
    """

    def test_orthogonal_x_y(self, fresh_sc):
        assert fresh_sc.dot_product(
                [None, 1, 0, 0], [None, 0, 1, 0]) == pytest.approx(0.0)

    def test_orthogonal_x_z(self, fresh_sc):
        assert fresh_sc.dot_product(
                [None, 1, 0, 0], [None, 0, 0, 1]) == pytest.approx(0.0)

    def test_self_dot_unit(self, fresh_sc):
        assert fresh_sc.dot_product(
                [None, 1, 0, 0], [None, 1, 0, 0]) == pytest.approx(1.0)

    def test_self_dot_general(self, fresh_sc):
        # [1,2,3]·[1,2,3] = 1 + 4 + 9 = 14
        assert fresh_sc.dot_product(
                [None, 1, 2, 3], [None, 1, 2, 3]) == pytest.approx(14.0)
    ...
    def test_general_vectors(self, fresh_sc):
        # [2, 3, 1]·[1, -1, 4] = 2 - 3 + 4 = 3
        assert fresh_sc.dot_product(
                [None, 2, 3, 1], [None, 1, -1, 4]) == pytest.approx(3.0)
    ...
    def test_float_inputs(self, fresh_sc):
        assert fresh_sc.dot_product(
                [None, 0.5, 0.5, 0.0],
                [None, 1.0, 1.0, 0.0]) == pytest.approx(1.0)


class TestCrossProduct:
    """cross_product(a, b) → ``[None, cx, cy, cz]``.

    Inputs and the returned list are 1-indexed.  Slot 0 of the result
    is ``None`` (Perl's ``$product[0] = 0.0`` sentinel analog).
    """

    def test_x_cross_y_is_z(self, fresh_sc):
        result = fresh_sc.cross_product([None, 1, 0, 0], [None, 0, 1, 0])
        assert result[1:] == pytest.approx([0, 0, 1])

    def test_y_cross_x_is_neg_z(self, fresh_sc):
        result = fresh_sc.cross_product([None, 0, 1, 0], [None, 1, 0, 0])
        assert result[1:] == pytest.approx([0, 0, -1])

    def test_x_cross_z_is_neg_y(self, fresh_sc):
        result = fresh_sc.cross_product([None, 1, 0, 0], [None, 0, 0, 1])
        assert result[1:] == pytest.approx([0, -1, 0])

    def test_z_cross_x_is_y(self, fresh_sc):
        result = fresh_sc.cross_product([None, 0, 0, 1], [None, 1, 0, 0])
        assert result[1:] == pytest.approx([0, 1, 0])

    def test_self_cross_is_zero(self, fresh_sc):
        result = fresh_sc.cross_product([None, 1, 2, 3], [None, 1, 2, 3])
        assert result[1:] == pytest.approx([0, 0, 0], abs=1e-14)
    ...
    def test_anticommutative(self, fresh_sc):
        ...
        for axis in range(1, 4):
            assert axb[axis] == pytest.approx(-bxa[axis])
    ...
    def test_general_result(self, fresh_sc):
        ...
        # cy = 4*5 - 2*7 = 20 - 14 =  6
        # cz = 2*6 - 3*5 = 12 - 15 = -3
        result = fresh_sc.cross_product([None, 2, 3, 4], [None, 5, 6, 7])
        assert result[1:] == pytest.approx([-3, 6, -3])
...
class TestCrossProductMag:
    ...
    def test_unit_vectors_perp(self, fresh_sc):
        assert fresh_sc.cross_product_mag(
                [None, 1, 0, 0], [None, 0, 1, 0]) == pytest.approx(1.0)

    def test_parallel_vectors(self, fresh_sc):
        assert fresh_sc.cross_product_mag(
                [None, 1, 0, 0], [None, 2, 0, 0]) == pytest.approx(0.0, abs=1e-14)

    def test_scaled_vectors(self, fresh_sc):
        # |[3,0,0] × [0,4,0]| = |[0,0,12]| = 12
        assert fresh_sc.cross_product_mag(
                [None, 3, 0, 0], [None, 0, 4, 0]) == pytest.approx(12.0)

    def test_relation_to_cross_product(self, fresh_sc):
        a = [None, 1.0, 2.0, 3.0]
        b = [None, 4.0, 5.0, 6.0]
        c = fresh_sc.cross_product(a, b)
        expected_mag = math.sqrt(c[1]**2 + c[2]**2 + c[3]**2)
...
class TestGetPlaneNormal:
    ...
    Each of the three points is 1-indexed ``[None, x, y, z]``;
    the returned normal is likewise 1-indexed.
    """

    def test_xy_plane_normal_is_z(self, fresh_sc):
        n = fresh_sc.get_plane_normal(
                [None, 0, 0, 0], [None, 1, 0, 0], [None, 0, 1, 0])
        ...
        assert abs(n[3]) > 1e-10  # non-zero z component

    def test_xz_plane_normal_is_y(self, fresh_sc):
        n = fresh_sc.get_plane_normal(
                [None, 0, 0, 0], [None, 1, 0, 0], [None, 0, 0, 1])
        assert n[1] == pytest.approx(0.0, abs=1e-10)
        assert abs(n[2]) > 1e-10
        assert n[3] == pytest.approx(0.0, abs=1e-10)
...
class TestSetLatticeFromMagAngle:
    ...
    def test_cubic_mags(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 5.0, 5.0, 5.0], [None, 90.0, 90.0, 90.0])
    ...
    def test_hexagonal_gamma_120(self, fresh_sc):
        ...
        fresh_sc.set_lattice_from_mag_angle(
            [None, 2.694, 2.694, 4.393],
            [None, 90.0, 90.0, 120.0])
        fresh_sc.abc_alpha_beta_gamma()
    ...
    def test_orthorhombic_distinct_mags(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 3.0, 4.0, 5.0], [None, 90.0, 90.0, 90.0])
        fresh_sc.abc_alpha_beta_gamma()
...
class TestAbcAlphaBetaGamma:
    ...
    def test_cubic_angles_are_90(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 4.0, 4.0, 4.0], [None, 90.0, 90.0, 90.0])
        fresh_sc.abc_alpha_beta_gamma()
        for axis in range(1, 4):
...
class TestMakeInvOrRecipLattice:
    ...
    def test_cubic_inverse_is_diagonal(self, fresh_sc):
        ...
        a = 5.0
        fresh_sc.set_lattice_from_mag_angle([None, a, a, a], [None, 90.0, 90.0, 90.0])
...
class TestFractABC2DirectXYZ:
    """fract_abc2direct_xyz(fract).

    Input and output both use the module-wide
    ``[None, a, b, c]`` / ``[None, x, y, z]`` 1-indexed layout.
    """

    def test_origin_maps_to_origin(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 5.0, 5.0, 5.0], [None, 90.0, 90.0, 90.0])
        xyz = fresh_sc.fract_abc2direct_xyz([None, 0.0, 0.0, 0.0])
        assert xyz[1:] == pytest.approx([0.0, 0.0, 0.0], abs=1e-10)

    def test_cubic_half_half_half(self, fresh_sc):
        # In a cubic 5 Å cell, fract (0.5,0.5,0.5) → cart (2.5, 2.5, 2.5)
        fresh_sc.set_lattice_from_mag_angle([None, 5.0, 5.0, 5.0], [None, 90.0, 90.0, 90.0])
        xyz = fresh_sc.fract_abc2direct_xyz([None, 0.5, 0.5, 0.5])
        assert xyz[1:] == pytest.approx([2.5, 2.5, 2.5], rel=1e-8)

    def test_cubic_corner(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 3.0, 4.0, 5.0], [None, 90.0, 90.0, 90.0])
        xyz = fresh_sc.fract_abc2direct_xyz([None, 1.0, 1.0, 1.0])
        assert xyz[1:] == pytest.approx([3.0, 4.0, 5.0], rel=1e-8)


class TestDirectXYZ2FractABC:
    """direct_xyz2fract_abc(xyz).

    Input and output both use the module-wide
    ``[None, x, y, z]`` / ``[None, a, b, c]`` 1-indexed layout.
    """

    def test_origin_maps_to_origin(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 5.0, 5.0, 5.0], [None, 90.0, 90.0, 90.0])
        frac = fresh_sc.direct_xyz2fract_abc([None, 0.0, 0.0, 0.0])
        assert frac[1:] == pytest.approx([0.0, 0.0, 0.0], abs=1e-10)

    def test_cubic_corner(self, fresh_sc):
        fresh_sc.set_lattice_from_mag_angle([None, 3.0, 4.0, 5.0], [None, 90.0, 90.0, 90.0])
        frac = fresh_sc.direct_xyz2fract_abc([None, 3.0, 4.0, 5.0])
        assert frac[1:] == pytest.approx([1.0, 1.0, 1.0], rel=1e-8)

    def test_roundtrip_cubic(self, fresh_sc):
        """xyz → fract → xyz should recover the original coordinates."""
        fresh_sc.set_lattice_from_mag_angle([None, 4.0, 5.0, 6.0], [None, 90.0, 90.0, 90.0])
        xyz_orig = [None, 1.2, 3.4, 5.1]
        frac = fresh_sc.direct_xyz2fract_abc(xyz_orig)
        xyz_back = fresh_sc.fract_abc2direct_xyz(frac)
        assert xyz_back[1:] == pytest.approx(xyz_orig[1:], rel=1e-8)
