...
subroutine computeFieldMesh(inSCF)
   ...
   !   ranges included in the imago.dat file.
   ! The basic algorithm follows:

   ! A real-space mesh is defined according to a request from the user about
   !   the number of points along each of the a,b,c axes in the imago.dat
   !   input file.
...
!allocate (valeValeOLGamma(valeDim,valeDim))
   ...
   !   which is derived from the imago.dat input value of the BASISFUNCTION
...
!call flush (20)
            ...
            !   (GTO) is A * r^l * exp(-ar^2).  In Imago an atomic orbital is
