...
subroutine setPotControlParameters (fbL,lastIt,corrCode,rlxFact,cTest,&
   ...
   call get_environment_variable("IMAGO_DATA", dataDirectory, info)
...
subroutine initPotCoeffs
   ...
   !   from the file and keep the ones from the imago.dat input file. (Although
   ...
   do i = 1, spin
      ...
      do j = 1, numPotTypes
         ...
         if (tempInt /= potTypes(j)%numAlphas) then
            write (20,*) "Mismatch between scfV and imago.dat:"
            write (20,*) "scfV claims type ",j," has ",tempInt," terms"
            write (20,*) "imago.dat claims type ",j," has ",&
...
!#endif
   ...
   if (tempInt /= numPlusUJAtoms) then
      ...
      write (20,*) "imago.dat claims that there are ",numPlusUJAtoms,"."
      stop
   endif
   ...
   do j = 1, 2
      ...
      do k = 1, numPlusUJAtoms
         ...
         if (tempInt /= plusUJAtomID(k)) then
            ...
            write (20,*) "imago.dat claims that it is ", plusUJAtomID(k)
            stop
         endif
         do i = 1, numKPoints
!#ifndef GAMMA
