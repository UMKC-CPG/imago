...
subroutine makeECMeshAndOverlap
   ...
   use HDF5
   use O_SCFExchCorrHDF5, only: numPoints_did, numPoints, radialWeight_did, &
...
!endif
   ...
   call h5awrite_f(exchCorrGroup_aid,H5T_NATIVE_INTEGER,1,&
         & attribIntDims,hdferr)
   if (hdferr /= 0) stop 'Failed to record exchCorrGroup success'
   call h5aclose_f(exchCorrGroup_aid,hdferr)
   if (hdferr /= 0) stop 'Failed to close exchCorrGroup attribute'
