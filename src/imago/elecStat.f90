...
subroutine neutralAndNuclearQPot
   ...
   use HDF5
   use O_SCFElecStatHDF5, only: potAlphaOverlap_did, nonLocalNeutQPot_did, &
         & nonLocalNucQPot_did, localNeutQPot_did, localNucQPot_did, &
         & potOLAndLocalQ_aid, potPot, pot
   ...
   !
   ! A key aspect of Imago is that the charge density is cast into the form of
   ...
   ! Record that the calculations are complete.
   call h5awrite_f(potOLAndLocalQ_aid,H5T_NATIVE_INTEGER,1,&
         & attribIntDims,hdferr)
   if (hdferr /= 0) stop 'Failed to record potOLAndLocalQ success'
   call h5aclose_f(potOLAndLocalQ_aid,hdferr)
   if (hdferr /= 0) stop 'Failed to close potOLAndLocalQ attribute'
...
subroutine residualQ
   ...
   ! Import the necessary HDF modules
   use HDF5
   use O_SCFElecStatHDF5, only: nonLocalResidualQ_did, nonLocalResidualQ_aid,&
         & potTypesPot
...
!            enddo
   ...
   ! Record that the calculations are complete.
   call h5awrite_f(nonLocalResidualQ_aid,H5T_NATIVE_INTEGER,1,&
         & attribIntDims,hdferr)
   if (hdferr /= 0) stop 'Failed to record nonLocalResidualQ success'
   call h5aclose_f(nonLocalResidualQ_aid,hdferr)
   if (hdferr /= 0) stop 'Failed to close nonLocalResidualQ attribute'
